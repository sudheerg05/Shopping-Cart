...
myApp.controller("myController", function ($scope) {
      ...
      //  alert($scope.button_value);
        ...
        for (var i in $scope.products) {
            if ($scope.products[i].name == name) {
                $scope.button_value(i);
            }
        }
        for (var i in $scope.products2) {
            if ($scope.products2[i].name == name) {
                $scope.button_value1(i);
            }
        }
    }
    ...
    $scope.removeFromCart = function (index) {
        ...
        for (var i in $scope.products2) {
            if ($scope.products2[i].name == $scope.cartItems[index].nam) {
                ind = i
            }
        }
    ...
    $scope.button_value1 = function (index) {

        //alert("inside sub function      " + index +"------------"+ $scope.hiddenbtn[index]);
        if (angular.isUndefined($scope.button_value1[index])) {
            $scope.button_value1[index] = true;
        }
        else if (angular.equals($scope.button_value1[index], false)) {
            $scope.button_value1[index] = true;
            //$scope.items[i].itemval[index] = 1;
            //$scope.itemval[index] = 1;
        }
        else if (angular.equals($scope.button_value1[index], true)) {
            $scope.button_value1[index] = false;
        }
        else {
            $scope.button_value1[index] = false;
        }
